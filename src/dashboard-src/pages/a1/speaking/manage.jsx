...
export default function A1SpeakingManage() {
  const [chapters, setChapters] = useState([]);
  const [hasChanges, setHasChanges] = useState(false);
  const [deletingId, setDeletingId] = useState(null);
  const sensors = useSensors(useSensor(PointerSensor));
  useEffect(() => {
    getA1Chapters("speaking").then((res) => setChapters(res.data));
  }, []);
  const handleDelete = async (ch) => {
    if (!confirm(`Delete "${ch.chapter_name}"?`)) return;
    setDeletingId(ch.id);
    await deleteA1Chapter("speaking", ch.id);
  ...
  return (
    <div className="px-4 py-8 max-w-9xl mx-auto">
      <div className="mb-8 flex justify-between">
        <h1 className="text-2xl font-bold">Manage A1 Speaking</h1>
      </div>
      {chapters.length > 0 && (
