...
export default function A1GrammarManage() {
  const [chapters, setChapters] = useState([]);
  const [hasChanges, setHasChanges] = useState(false);
  const sensors = useSensors(useSensor(PointerSensor));

  useEffect(() => {
    getA1Chapters("grammar").then((res) => setChapters(res.data));
  }, []);

  const handleDelete = async (chapter) => {
    if (!window.confirm(`Delete "${chapter.chapter_name}"?`)) return;
    await deleteA1Chapter("grammar", chapter.id);
  ...
  return (
    <div className="px-4 py-8 max-w-9xl mx-auto">
      <h1 className="text-2xl font-bold mb-8">Manage A1 Grammar</h1>
      {chapters.length > 0 && (
