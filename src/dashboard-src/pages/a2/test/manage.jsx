...
export default function A2TestManage() {
  const [chapters, setChapters] = useState([]);
  const [hasChanges, setHasChanges] = useState(false);
  const sensors = useSensors(useSensor(PointerSensor));
  useEffect(() => {
    getA2Chapters("test").then((res) => setChapters(res.data));
  }, []);
  const handleDelete = async (ch) => {
    if (!confirm(`Delete "${ch.chapter_name}"?`)) return;
    await deleteA2Chapter("test", ch.id);
  ...
  return (
    <div className="px-4 py-8 max-w-9xl mx-auto">
      <h1 className="text-2xl font-bold mb-8">Manage A2 Test Topics</h1>
      {chapters.length > 0 && (
