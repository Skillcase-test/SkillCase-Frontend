...
export default function B1NewsManage() {
  ...
  const fetchChapters = async () => {
    setLoading(true);
    try {
      const res = await getB1Chapters("news");
      setChapters(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error(err);
      toast.error("Failed to fetch news topics");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, title) => {
    ...
    try {
      const res = await deleteB1Chapter("news", id);
      if (res.data?.success) {
  ...
  const handleMove = async (index, direction) => {
    ...
    try {
      await reorderB1Chapters("news", orderedIds);
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Reorder or delete existing B1 news topics.
          </p>
        </div>
        <button
                    ...
                    <td className="px-6 py-4">
                      <div className="w-16 h-12 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center">
                        {ch.hero_image_url ? (
                          ...
                          <span className="text-[10px] text-slate-400 font-bold uppercase">News</span>
                        )}
                      </div>
                    </td>
