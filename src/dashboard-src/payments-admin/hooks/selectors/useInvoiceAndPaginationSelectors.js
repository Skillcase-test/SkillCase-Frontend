...
export function useInvoiceAndPaginationSelectors(state, core) {
  ...
  const resolvedFeeSummary = useMemo(() => {
    if (tab !== "fee") return { unpaidSoFar: 0, paidSoFar: 0, potentialAfterDiscounts: 0, totalDiscounts: 0, activeButNotScheduled: 0, potentialLost: 0 };
    return {
      ...
      potentialLost: Number(feeSummary?.potential_lost_paise || 0),
    };
  }, [feeSummary, tab]);
