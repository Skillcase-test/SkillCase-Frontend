...
// Multi-select variant of DynamicDropdownField: the same managed options
// dropdown, but picks multiple tags where exactly one is the primary
// (primary is always inside `values`; first tag selected becomes primary,
// removing the primary promotes the first remaining tag).
export function DynamicMultiTagField({
  label,
  field,
  values = [],
  primaryValue = "",
  ...
  const emit = (nextValues, nextPrimary) => {
    const primary = nextValues.includes(nextPrimary)
      ? nextPrimary
      : nextValues[0] || "";
  ...
  const toggleTag = (tag) => {
    if (values.includes(tag)) {
      emit(
        ...
        primaryValue === tag ? "" : primaryValue,
      );
    } else {
      emit([...values, tag], primaryValue || tag);
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (!values.includes(trimmed)) {
        emit([...values, trimmed], primaryValue || trimmed);
      }
      setSearchQuery("");
      toast.success(`Option "${trimmed}" added`);
    } catch (err) {
  ...
  const handleUpdate = async (e, id) => {
    ...
    try {
      await onUpdateOption(id, trimmed);
      const targetOpt = options.find((o) => o.id === id);
      if (targetOpt && values.includes(targetOpt.option_value)) {
        emit(
          values.map((v) => (v === targetOpt.option_value ? trimmed : v)),
          primaryValue === targetOpt.option_value ? trimmed : primaryValue,
        );
      }
      setEditingId(null);
      setEditingVal("");
    ...
    try {
      ...
      if (values.includes(optVal)) {
        emit(
          ...
          primaryValue === optVal ? "" : primaryValue,
        );
      }
      toast.success(`Option "${optVal}" removed`);
    } catch (err) {
    ...
    <div className="space-y-1 relative" ref={wrapRef}>
      ...
      {!readOnly && (
        <div className="relative">
          <button
            ...
            className={`w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-left flex items-center justify-between outline-none transition focus:border-slate-400 focus:outline-none focus:ring-0 ${
              open ? "border-slate-400" : ""
            }`}
            style={{ outline: "none", boxShadow: "none" }}
          >
            <span
              className={`truncate ${
                values.length ? "text-slate-900 font-medium" : "text-slate-400"
              }`}
            >
              {values.length
                ? `${values.length} selected${primaryValue ? ` · Primary: ${primaryValue}` : ""}`
                : displayPlaceholder}
            </span>
            <ChevronDown
              className={`w-4 h-4 text-slate-400 shrink-0 ml-2 transition-transform duration-200 ${
      ...
      {values.length ? (
        <div className="flex flex-wrap gap-1.5 pt-1">
          {values.map((tag) => {
            const isPrimary = tag === primaryValue;
            return (
              <span
                key={tag}
