...
export default function A1Speaking() {
  ...
  // Tour integration
  const { isTourActive, currentFeature, speakingStep } = useA1Tour();
  ...
  const handleTourStartRecording = () => {
    ...
    if (isTourMode && speakingStep === 1) {
      window.dispatchEvent(
        new CustomEvent("tour:A1SpeakingStep", { detail: { step: 2 } }),
      );
    }
  };
  ...
  const handleTourStopRecording = () => {
    ...
    if (isTourMode && speakingStep === 2) {
      window.dispatchEvent(
        new CustomEvent("tour:A1SpeakingStep", { detail: { step: 3 } }),
      );
    }
  ...
  useEffect(() => {
    if (isTourMode && speakingStep === 3 && assessmentResult) {
      window.dispatchEvent(
        new CustomEvent("tour:A1SpeakingStep", { detail: { step: 4 } }),
      );
    }
  ...
  const handleNext = async () => {
    cancelSpeech();
    if (currentIndex < totalCards - 1 && !guardUsage("A1", "speaking")) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 relative overflow-hidden">
        {loading ? (
          ...
          <SpeakingCardDeck
            ...
            containerId="A1-speaking-container"
          />
        )}
      </div>
