...
import { getTestProgress } from "../../../api/a1Api";

export default function A1TestLevel() {
  ...
  const handleLevelClick = (level, isCompleted) => {
    if (isCompleted) {
      ...
      navigate(`/a1/test/${topicId}/${level}?mode=review`);
    } else {
      // Take test for current/accessible level
      navigate(`/a1/test/${topicId}/${level}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="p-6">
        ...
        <div id="A1-test-levels" className="space-y-3">
          {[1, 2, 3, 4, 5].map((level) => {
            // FIX: Correct logic
