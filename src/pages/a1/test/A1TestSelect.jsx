...
export default function A1TestSelect() {
  ...
  useEffect(() => {
    if (showModal) {
      window.dispatchEvent(new Event("tour:A1TestPrerequisite"));
    }
  }, [showModal]);
  useEffect(() => {
    if (!user) {
  ...
  const handleTopicClick = (topic) => {
    ...
    if (prerequisites.length > 0) {
      ...
      navigate(`/a1/test/${topic.topic_id}`);
    }
  };
  const handleModalDismiss = () => {
    setShowModal(false);
    navigate(`/a1/test/${selectedTopic.topic_id}`);
  };
    ...
    <>
      ...
      {showModal && selectedTopic && (
        <A2TestPrerequisiteModal
          ...
          modalId="A1-test-prerequisite-modal"
          gotItButtonId="A1-test-got-it-btn"
        />
      )}
    </>
  );
