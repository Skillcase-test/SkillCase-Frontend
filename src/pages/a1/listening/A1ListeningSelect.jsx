...
import { getListeningChapters } from "../../../api/a1Api";
import { useUsageLimitModule } from "../../../hooks/useUsageLimits";
export default function A1ListeningSelect() {
  const navigate = useNavigate();
  const { user } = useSelector((state) => state.auth);
  const [chapters, setChapters] = useState([]);
  const [loading, setLoading] = useState(true);
  const { locked: usageLocked } = useUsageLimitModule("A1", "listening");
  useEffect(() => {
    if (!user) {
      navigate("/login");
      return;
  ...
  const handleChapterClick = (chapter) => {
    navigate(`/a1/listening/${chapter.id}`);
  };
