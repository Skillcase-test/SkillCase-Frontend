...
export default function A1Reading() {
  ...
  const screenWindow = (() => {
    if (contentList.length <= 2) {
      return Array.from({ length: contentList.length }, (_, i) => i);
    }
    if (currentIndex === 0) return [0, 1];
    if (currentIndex === contentList.length - 1)
      return [contentList.length - 2, contentList.length - 1];
  ...
  if (loading) {
    return <ExerciseLayoutSkeleton variant="reading" title="Reading Practice" />;
  }
  if (!currentContent) {
    return (
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="px-4 py-2.5 flex items-center justify-between border-b border-gray-100 gap-2">
        ...
        <div className="flex items-center gap-1">
          {currentIndex > 0 && (
            <button
              onClick={() => requestScreenChange(currentIndex - 1)}
          ...
          {screenWindow.map((idx) => (
            <button
              key={idx}
              onClick={() => requestScreenChange(idx)}
