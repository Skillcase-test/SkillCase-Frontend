...
import { getReadingChapters } from "../../../api/a2Api";
import { useUsageLimitModule } from "../../../hooks/useUsageLimits";
export default function A2ReadingSelect() {
  const navigate = useNavigate();
  const { user } = useSelector((state) => state.auth);
  const [chapters, setChapters] = useState([]);
  const [loading, setLoading] = useState(true);
  const { locked: usageLocked } = useUsageLimitModule("A2", "reading");
  useEffect(() => {
    if (!user) {
      navigate("/login");
      return;
  ...
  const handleChapterClick = (chapter) => {
    navigate(`/a2/reading/${chapter.id}`);
  };
