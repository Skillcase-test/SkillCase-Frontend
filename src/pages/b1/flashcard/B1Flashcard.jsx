...
export default function B1Flashcard() {
  ...
  useEffect(() => {
    ...
    if (justFlipped) {
      window.dispatchEvent(new Event("tour:b1FlashcardRevealed"));
    }
  ...
  const skipTest = () => {
    ...
    if (currentCard < totalCards - 1) {
      if (!guardUsage("B1", "flashcard")) return;
      setCurrentCard(currentCard + 1);
      setDeckRotation((p) => (p + 1) % 3);
      setIsFlipped(false);
    }
  ...
  const continueAfterTest = () => {
    setShowTest(false);
    if (isFinalTest) navigate("/b1/flashcard");
    else {
      if (!guardUsage("B1", "flashcard")) return;
      setCurrentCard(currentCard + 1);
      setDeckRotation((p) => (p + 1) % 3);
          ...
          {!testSubmitted ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 max-w-md mx-auto">
              ...
              {testResults.passed ? (
                ...
                >
                  {isAtFinalTest ? "Complete Chapter" : "Continue"}
                </button>
              ) : (
