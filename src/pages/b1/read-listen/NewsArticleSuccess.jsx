...
export default function NewsArticleSuccess() {
  ...
  const handleReview = () => {
    navigate(`/b1/read-listen/content/${contentId}`, {
      state: {
        reviewMode: true,
        answers,
        results,
  ...
  const handleTryNext = async () => {
    try {
      ...
      if (nextTopic) {
        navigate(`/b1/read-listen/content/${nextTopic.id}`);
      } else {
        const currentIdx = list.findIndex((t) => t.id === contentId);
