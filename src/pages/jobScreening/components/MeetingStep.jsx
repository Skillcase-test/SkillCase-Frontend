...
const MeetingStep = ({ type, progress, onComplete }) => {
  ...
  const slotTime = isTraining ? progress?.training_slot_time : progress?.recruiter_slot_time;
  const meetLink = isTraining ? progress?.training_meet_link : progress?.recruiter_meet_link;
  ...
  const isFailed = isTraining ? progress?.training_completed === false : progress?.recruiter_interview_passed === false;
    ...
    return (
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm/30 p-6 sm:p-8 flex flex-col items-center justify-center text-center w-full font-sans">
        ...
        <h2 className="text-xl sm:text-2xl font-extrabold text-red-600 tracking-tight mb-2">
          {isTraining ? "Mentoring Session Failed" : "Recruiter Interview Failed"}
        </h2>
        <p className="text-zinc-500 text-xs sm:text-sm max-w-sm mx-auto leading-relaxed mb-6">
          Unfortunately, you did not pass the {isTraining ? "mentoring/training session" : "recruiter interview"} checkpoint. Please reach out to Skillcase for details.
        </p>
        <button
          type="button"
    ...
    return (
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm/30 p-6 sm:p-8 flex flex-col items-center justify-center text-center w-full font-sans">
        ...
        <h2 className="text-xl sm:text-2xl font-extrabold text-[#002856] tracking-tight mb-2">
          {isTraining ? "Scheduling Training Session" : "Scheduling Recruiter Interview"}
        </h2>
        <p className="text-zinc-500 text-xs sm:text-sm max-w-sm mx-auto leading-relaxed mb-6">
          Our scheduling coordinators are assigning a slot and setting up a Google Meet link for you.
          Please check back in a few minutes or click the refresh button below to sync updates.
        </p>
        <button
          onClick={handleRefresh}
          disabled={loading}
    ...
    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm/30 p-6 sm:p-8 flex flex-col items-center justify-center text-center w-full font-sans">
      ...
      <div className="w-full max-w-xs border border-slate-100 rounded-2xl p-4 bg-slate-50/20 mb-6 flex flex-col gap-3.5 text-left">
        <div className="flex items-start gap-3">
          ...
          <div>
            <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block mb-0.5">Date</span>
            <span className="text-xs sm:text-sm font-semibold text-slate-800">{formattedDate}</span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest block mb-0.5">Time</span>
            <span className="text-xs sm:text-sm font-semibold text-slate-800">{formattedTime}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 w-full sm:max-w-xs">
        {meetLink ? (
          ...
          <div className="p-3 bg-yellow-50 text-yellow-800 rounded-xl border border-yellow-100 text-xs font-semibold text-center leading-relaxed">
            Meeting link is being generated. It will appear here shortly before the slot time.
          </div>
        )}
