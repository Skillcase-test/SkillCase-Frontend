...
export default function SpeakingResults() {
  ...
  useEffect(() => {
    ...
    const userAudio = ansKey ? sectionData?.answers?.[ansKey] : null;
  ...
  const flatQuestions = questions.map((block) => {
    // Stored under the same "<blockIdx>_0" key the workspace submits with.
    const ansKey = `${block.id}_0`;
    const userAns = answersMap[ansKey] || {};
    const report =
      (feedbackData.questions
        ? feedbackData.questions[ansKey]
        : feedbackData[ansKey]) || {};
    const qScore =
