...
import {
  getB2Exercise,
  submitB2ExerciseSpeakingAnswer,
...
export default function SpeakingWorkspace() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      // Guard against a cross-module deep link (e.g. /b2/speaking/<reading-id>)
      if (exercise.module && exercise.module !== "speaking") {
        setFetchError(true);
        return;
      }
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      // Per-prompt speaking limit from the JSON item (fallback 60s).
      const speechLimit =
        Number(
          questions[currentBlockIndex]?.questions?.[0]?.speech_time_limit,
        ) || 60;
  ...
  const handleNext = async () => {
    ...
    if (audioBlob) {
      ...
      // Blocks map to id: blockIdx and the UI renders questions[0] per block,
      // so the addressing key is "<blockIdx>_0" — matches the backend's
      // questionKey contract and the stored answers/feedback keys.
      formData.append("questionKey", `${currentBlock.id}_0`);
  ...
  const finalizeSpeakingTest = async () => {
    ...
    setIsEvaluating(true);
    try {
      // Dedicated finalize endpoint (no body) once all recordings are in
      await submitB2ExerciseSpeaking(exerciseId);
        ...
        <div className="w-full flex flex-col items-center justify-center gap-5 pt-4 select-none">
          ...
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className="text-sky-950 text-xs font-semibold">
              {formatSeconds(recordDuration)} / {formatSeconds(speechLimit)}
            </span>
          </div>
