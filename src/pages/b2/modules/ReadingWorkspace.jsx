...
export default function ReadingWorkspace() {
  ...
  const fetchContent = async () => {
    ...
    setFetchError(false);
    try {
      // Standalone exercise — no session handshake; content.blocks carry the
      // same shape as exam-section question rows, keyed by block index.
      const res = await getB2Exercise(exerciseId);
      const exercise = res.data || {};
      // Guard against a cross-module deep link (e.g. /b2/reading/<writing-id>)
      if (exercise.module && exercise.module !== "reading") {
        setFetchError(true);
        return;
      }
...
function ClozeTestLayout({ block, answers, onSelect }) {
  ...
  const [showFirstTimeHint, setShowFirstTimeHint] = useState(() => {
    try {
      return !localStorage.getItem("b2_reading_cloze_guide_shown");
    } catch {
      return false;
    }
  });
