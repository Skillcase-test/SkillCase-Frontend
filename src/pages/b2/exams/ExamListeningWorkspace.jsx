...
export default function ExamListeningWorkspace() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      const contentRes = await getB2ExamSectionContent(paperId, "listening");
      const list = Array.isArray(contentRes.data) ? contentRes.data : [];
      setQuestions(list);

      if (list.length > 0 && list[0].duration_minutes) {
        const timerKey = `b2_exam_timer_${user?.user_id || "guest"}_${paperId}_listening`;
