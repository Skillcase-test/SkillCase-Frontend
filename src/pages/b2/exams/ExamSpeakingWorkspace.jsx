...
export default function ExamSpeakingWorkspace() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (list.length > 0 && list[0].duration_minutes) {
        const timerKey = `b2_exam_timer_${user?.user_id || "guest"}_${paperId}_speaking`;
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      // Per-prompt speaking limit from the section item (fallback 60s).
      const speechLimit =
        Number(
  ...
  const finalizeSpeakingTest = async () => {
    ...
    try {
      ...
      await submitB2ExamSpeakingAudio(submission.id, new FormData());
      localStorage.removeItem(
        `b2_exam_timer_${user?.user_id || "guest"}_${paperId}_speaking`,
      );
      navigate(`/b2/exams/papers/${paperId}/speaking/results`, {
        state: { submissionId: submission.id },
      });
