...
const isStepNotesCompatible = (appVersion) => {
  if (appVersion === null || appVersion === undefined || appVersion === "") {
    return true; // web user
  }
...
const CandidateDetail = ({
  ...
  const handleOpenStepNoteModal = (stepId, stepTitle, initialMessage = "") => {
    openConfirmModal({
  ...
  const handleDeleteStepNote = (stepId, stepTitle) => {
    openConfirmModal({
      title: `Delete Note for "${stepTitle}"?`,
      message:
        "This will remove the reviewer note from this step for the candidate.",
      onConfirm: () => {
        onUpdate(candidate.user_id, {
          clear_step_note_id: stepId,
        });
      },
    });
  };
        ...
        <div className="md:col-span-7 overflow-y-auto p-6 bg-white h-full">
          ...
          <div className="relative border-l-2 border-slate-100 ml-4 pl-8 space-y-6 py-2">
            {steps.map((step, index) => {
              ...
              const stepNotes = candidate?.step_notes || {};
              const currentStepNote = stepNotes[step.id];
                            ...
                            <>
                              <div className="flex items-center justify-between">
                                ...
                                {!currentStepNote && (
                                  <button
                                    type="button"
                                    disabled={!isNotesSupported || !canEdit}
                                    onClick={() =>
                              ...
                              {!isNotesSupported && (
                                <div className="flex items-start gap-1.5 p-2 bg-amber-50/90 border border-amber-200 rounded-lg text-[10px] text-amber-900 leading-snug">
                                  <AlertCircle className="w-3.5 h-3.5 text-amber-600 shrink-0 mt-0.5" />
                                  <span>
                                    App Version Incompatible (v
                                    {candidate?.app_version || "unknown"} &lt;
                                    v1.2.5) — Candidate cannot view step notes
                              ...
                              {currentStepNote && (
                                <div className="bg-amber-50/80 border border-amber-200/90 rounded-xl p-2.5 flex flex-col gap-2 text-[11px] text-amber-950">
                                  <div className="flex items-start justify-between gap-2">
                                    <p className="leading-relaxed whitespace-pre-wrap font-medium flex-1">
                                      {currentStepNote.message}
                                    </p>
                                    <span
                                      className={`px-1.5 py-0.5 rounded-full text-[8px] font-bold uppercase shrink-0 ${
                                        currentStepNote.viewed_at
                                  ...
                                  <div className="flex items-center justify-between pt-1 border-t border-amber-200/60 text-[9px] text-amber-800/80">
                                    <span>
                                      {currentStepNote.created_by?.name
                                        ? `By ${currentStepNote.created_by.name}`
                                        : ""}
                                      {currentStepNote.updated_at
                                        ? ` · ${new Date(currentStepNote.updated_at).toLocaleDateString()}`
                                        : ""}
                                    </span>
                                    <div className="flex items-center gap-2">
                                      {isNotesSupported && canEdit ? (
