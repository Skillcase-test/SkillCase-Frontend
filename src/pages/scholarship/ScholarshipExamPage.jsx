...
const FEATURE_KEY = "scholarship_exam";
// Mirrors PROCTOR_GRACE_MS in the backend controller. The server is the judge —
// this only stops us from bothering it about a half-second focus blip.
const PROCTOR_GRACE_MS = 3000;
...
// MAIN EXAM PAGE — standalone scholarship exam-taking screen
export default function ScholarshipExamPage() {
  ...
  // Latest answers, readable from event handlers that close over stale state
  // (submit-on-timeout, pagehide flush).
  const answersRef = useRef({});
  // Timestamp the candidate left the exam screen; null while they are present.
  ...
  // ---- PROCTORING ----
  // The candidate leaving the screen is reported, not judged: the client sends
  // how long they were away and the server decides whether it counts. That way
  // an incoming call, a permission sheet or the OS stealing focus for a moment
  // never costs a strike, and the rule lives in one place.
  //
  // Native uses Capacitor's appStateChange (a webview also fires
  // visibilitychange, so registering both would double-count every switch);
  // web uses visibilitychange.
  useEffect(() => {
    if (examClosed) return;

    const markAway = () => {
      if (isSubmittingRef.current || awayStartRef.current != null) return;
      awayStartRef.current = Date.now();
      // Debounced answer saves would be lost if the app is killed while away.
      flushPendingSaves();
    };
    ...
    const markBack = () => {
      ...
      if (start == null || isSubmittingRef.current) return;
      const awayMs = Date.now() - start;
      // Below the grace window the server would forgive it anyway — save a call.
      if (awayMs < PROCTOR_GRACE_MS) return;
      triggerWarning("Left the exam screen", awayMs);
    };
    ...
    if (Capacitor.isNativePlatform()) {
      stateListener = CapApp.addListener("appStateChange", ({ isActive }) =>
        isActive ? markBack() : markAway(),
      );
      // Back would drop them out of the exam mid-attempt — confirm instead, and
      // never treat it as a violation.
    ...
    // Closing the tab is not a strike (the sweeper grades the attempt), but any
    // answer still sitting in a debounce timer must reach the server first.
    const handlePageHide = () => flushPendingSaves();
    // Native/web both keep the browser's own "leave site?" prompt.
    ...
    return () => {
      ...
      if (stateListener?.remove) stateListener.remove();
      else if (typeof stateListener?.then === "function")
        stateListener.then((h) => h.remove());
      if (backHandler?.remove) backHandler.remove();
      else if (typeof backHandler?.then === "function")
  ...
  const triggerWarning = useCallback(
    async (reason, awayMs) => {
      ...
      try {
        ...
        const data = res.data;
        // The server forgave this absence — nothing to show the candidate.
  ...
  // Fire every pending debounced answer save now. Used when the candidate
  // backgrounds the app or closes the tab — the debounce timer would never run.
  const flushPendingSaves = useCallback(() => {
    const pending = Object.keys(saveTimeoutRef.current);
  ...
  const handleAutoSubmit = async (reason) => {
    ...
    try {
      // Send the answers along: a debounce timer that never fired must not cost
      // the candidate a question.
      await submitExam(testId, answersRef.current);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 text-center">
        ...
        <p className="text-gray-500 mb-6 max-w-sm">
          {closedReason === "submitted"
            ? "Your answers have been recorded. The scholarship team is reviewing submissions and results will be announced soon."
            : closedReason}
        </p>
        <button
          onClick={() => navigate("/scholarship")}
    ...
    <div className="min-h-screen bg-white flex flex-col">
      <div className="sticky z-40">
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="text-xs font-bold text-[#002856] bg-[#edfaff] px-3 py-1.5 rounded-lg shrink-0"
            >
        ...
        <div className="h-1.5 bg-gray-200">
          <div
            className="h-full bg-gradient-to-r from-[#002856] to-[#01347a] transition-all duration-300"
            style={{
              width: `${
        ...
        <div className="bg-white px-4 py-1.5 flex items-center justify-between text-[11px] text-gray-400">
          <span>
            {answeredCount}/{answerableQuestions.length} answered
          </span>
          <span className="font-medium text-gray-500">Scholarship Exam</span>
        </div>
      </div>

      {/* Page Content — multiple questions per page */}
      <div ref={pageContentRef} className="p-4 pb-10 max-w-md mx-auto w-full">
        <div className="space-y-5 mt-2">
          {(() => {
            return currentPage.map((q) => {
              ...
              if (q.question_type === "image_block") {
                return (
                  <div
                    key={q.question_id}
                    className="rounded-xl px-3 py-3 bg-white border border-[#e5e9f0] shadow-sm mb-1 flex justify-center"
                  >
              ...
              if (q.question_type === "reading_passage") {
                return (
                  <div
                    key={q.question_id}
                    className="rounded-xl bg-white border border-[#e5e9f0] shadow-sm p-4"
                  >
              ...
              if (q.question_type === "paragraph") {
                ...
                return (
                  <div
                    key={q.question_id}
                    className="bg-white rounded-2xl border border-[#e5e9f0] shadow-sm p-5"
                  >
                    <div className="inline-flex items-center gap-1.5 mb-3">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-[#002856] bg-[#edfaff] px-2 py-1 rounded-md">
                ...
                >
                  <div className="inline-flex items-center gap-1.5 mb-3">
                    ...
                    {answer !== undefined && answer !== null && (
                      <span className="text-[10px] font-bold uppercase tracking-wider text-green-600 bg-green-50 px-2 py-1 rounded-md">
                        Answered ✓
                      </span>
                    )}
                  </div>
      ...
      {/* Leave Confirm Modal — native back button */}
      {showLeaveConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl p-6 max-w-sm mx-4 shadow-2xl w-full">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-[#002856]">Leave the exam?</h3>
                <p className="text-sm text-gray-500">
                  Your timer keeps running while you are away, and leaving the
                  screen can count as a violation.
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-[#002856] text-white rounded-xl font-semibold"
              >
                Stay
              </button>
              <button
                ...
                className="flex-1 py-3 border border-[#002856] text-[#002856] rounded-xl font-semibold"
              >
                Leave
              </button>
            </div>
